...
router.get('/stats', async (req, res) => {
  try {
    const [statsResult] = await db.query(
      "SELECT COUNT(*) as total_orders, SUM(CASE WHEN status = 'Pending' THEN 1 ELSE 0 END) as pending_orders, SUM(CASE WHEN status = 'Ready' THEN 1 ELSE 0 END) as ready_orders, SUM(total_amount) as total_revenue FROM orders"
    );
